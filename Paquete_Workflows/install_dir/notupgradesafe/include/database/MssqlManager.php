            ...
            if (!empty($matches[3])) {
                if ($start == 0) {
					...
					*/
					...
					if(substr_count(strtolower($matches[0]), 'distinct') >
						(substr_count(strtolower($matches[0]), 'distinct secr.record_id')
						+ substr_count(strtolower($matches[0]), 'distinct secg.id')
						+ substr_count(strtolower($matches[0]), 'distinct sec.user_id')
						)
					){
						$hasDistinct = true;
					}
					/* END [IC] - SECURITY GROUPS */
                    if ($hasDistinct) {
